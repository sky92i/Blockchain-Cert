...
export const MyDocumentsListPage = () => {
  ...
  useEffect(() => {
    const retrieveDocuments = async (email) => {
      const accessToken = await getAccessTokenSilently();
      await UserService.getAll(email, accessToken)
        .then((response) => {
  ...
  const downloadFile = async () => {
    const accessToken = await getAccessTokenSilently();
    await UserService.downloadFile(currentDocument.file, accessToken)
      .then((response) => {
        //save file to client
  ...
  const searchTitleFunction = async () => {
    ...
    await UserService.findByTitle(searchTitle, user.email, accessToken)
      .then((response) => {
        setDocuments(response.data);
        console.log(response.data);
      })
